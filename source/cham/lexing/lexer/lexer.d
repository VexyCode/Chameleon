...
private:
    ...
    Token readNumber()
    {
        ...
        while (isNumber(cur) || cur == '.')
        {
            ...
            {
                if (isFloat)
                    throwLexerError("Multiple decimal points in float", Token(line, col, ".", TT
                            .Unk));
                isFloat = true;
            }
            advance();
    ...
    Token nextToken()
    {
        ...
        if (isEnd())
            return Token(line, col, "", TT.Eof);

        else if (cur.isAlpha() || cur == '_')
            ret = readLetters();
        ...
        else if (isOp(cur.to!string()))
        {
            ...
            ret = Token(line, col, cur.to!string(), TT.Unk); // fallback unknown
    ...
    void skipComment()
    {
        if (cur == '/' && peek() == '/')
        {
            // single line comment, skip to end of line or file
            while (!isEnd() && cur != '\n')
            {
                advance();
            }
        ...
        {
            // multi-line comment, skip until you find */
            advance(); // consume '/'
            advance(); // consume '*'
            while (!isEnd())
            {
            ...
            if (isEnd())
            {
                throwLexerError("Unterminated multi-line comment", Token(line, col, "", TT.Unk));
            }
        }
    }
    ...
    Token readPunct()
    {
        ...
        default:
            throwLexerError("Unknown punctuation: " ~ cur.to!string,
                Token(line, col, cur.to!string(), TT.Unk));
            assert(0);
        }
    }
    ...
    /// Returns the character at an offset related to the current index
    ...
    void throwLexerError(string msg, Token token)
    {
        string ln = (token.line >= 1 && token.line <= src.splitLines().length)
            ? src.splitLines()[token.line - 1] : "<line unavailable>";
